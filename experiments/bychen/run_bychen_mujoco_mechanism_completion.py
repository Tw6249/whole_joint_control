...
from scripts.reporting.report_paths import analysis_report_dir, repo_relpath
...
def run_mujoco(config: Path, out_dir: Path, args: argparse.Namespace) -> bool:
    ...
    cmd = [
        sys.executable,
        "scripts/simulation/run_mujoco.py",
...
def write_summary(out_dir: Path, metrics: pd.DataFrame, freq: pd.DataFrame, args: argparse.Namespace) -> Path:
    ...
    lines = [
        ...
        "",
        "Scope: all rows in this folder were generated with `scripts/simulation/run_mujoco.py` and "
...
def main() -> int:
    ...
    parser.add_argument("--base-config", type=Path, default=Path("experiments/hip_knee/configs/h1_real_p4_ku_u1_hip_knee_eid.yaml"))
