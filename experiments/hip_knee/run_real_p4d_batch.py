#!/usr/bin/env python3
"""Run P4D real H1 EID parameter scans with software disturbance.

P4D mirrors the existing P4 Ko/Ku scan workflow, but every run receives the
same repeatable hip-knee software torque pulse. The original P4 batch script is
...
DEFAULT_OUT_DIR = REPO_ROOT / "analysis_artifacts" / "real_p4d_batch"
...
KO_SPECS = [
    ParamSpec("o0", "ko", "Ko_o0", REPO_ROOT / "experiments" / "hip_knee" / "configs" / "h1_real_p4_ko_o0_hip_knee_eid.yaml", 0.0, 1.0),
    ParamSpec("o1", "ko", "Ko_o1", REPO_ROOT / "experiments" / "hip_knee" / "configs" / "h1_real_p4_ko_o1_hip_knee_eid.yaml", 0.25, 1.0),
    ParamSpec("o2", "ko", "Ko_o2", REPO_ROOT / "experiments" / "hip_knee" / "configs" / "h1_real_p4_ko_o2_hip_knee_eid.yaml", 0.5, 1.0),
    ParamSpec("o3", "ko", "Ko_o3", REPO_ROOT / "experiments" / "hip_knee" / "configs" / "h1_real_p4_ko_o3_hip_knee_eid.yaml", 0.75, 1.0),
    ParamSpec("o4", "ko", "Ko_o4", REPO_ROOT / "experiments" / "hip_knee" / "configs" / "h1_real_p4_ko_o4_hip_knee_eid.yaml", 1.0, 1.0),
    ParamSpec("o5", "ko", "Ko_o5", REPO_ROOT / "experiments" / "hip_knee" / "configs" / "h1_real_p4_ko_o5_hip_knee_eid.yaml", 1.25, 1.0),
]

KU_SPECS = [
    ParamSpec("u1", "ku", "Ku_u1", REPO_ROOT / "experiments" / "hip_knee" / "configs" / "h1_real_p4_ku_u1_hip_knee_eid.yaml", 1.0, 0.5),
    ParamSpec("u2", "ku", "Ku_u2", REPO_ROOT / "experiments" / "hip_knee" / "configs" / "h1_real_p4_ku_u2_hip_knee_eid.yaml", 1.0, 0.75),
    ParamSpec("u3", "ku", "Ku_u3", REPO_ROOT / "experiments" / "hip_knee" / "configs" / "h1_real_p4_ku_u3_hip_knee_eid.yaml", 1.0, 1.0),
    ParamSpec("u4", "ku", "Ku_u4", REPO_ROOT / "experiments" / "hip_knee" / "configs" / "h1_real_p4_ku_u4_hip_knee_eid.yaml", 1.0, 1.25),
]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run P4D real H1 EID Ko/Ku parameter scans with software torque disturbance."
    )
    parser.add_argument("--execute", action="store_true", help="Actually run h1_direct. Default is dry-run.")
    parser.add_argument("--yes", action="store_true", help="Skip the RUN_P4D confirmation prompt.")
    ...
    parser.add_argument(
        ...
        choices=["ko", "ku", "both"],
        default="ko",
        help="Parameter scan to run. Default only scans Ko.",
    )
    parser.add_argument(
        "--groups",
        default="",
        help="Comma-separated parameter groups overriding --scan, e.g. o4,o5 or u2,u3,u4.",
    )
    parser.add_argument(
        "--order",
...
def validate_args(args: argparse.Namespace, specs: list[ParamSpec]) -> None:
    ...
    if args.disturbance_method.strip() in {"", "none"}:
        raise SystemExit("--disturbance-method must be non-empty for P4D")
...
def condition_for(spec: ParamSpec) -> str:
    return f"P4D_EID_{spec.label}_anti_phase_hip_knee_software_disturbance"
...
def confirm_or_exit(args: argparse.Namespace, plan: list[tuple[int, ParamSpec]]) -> None:
    print(
        f"P4D batch plan: {len(plan)} runs, duration={args.duration:g}s, pause={args.pause:g}s, "
        ...
        f"{args.disturbance_plateau_end:g}/{args.disturbance_end:g}, order={args.order}"
    )
    ...
    answer = input("Type RUN_P4D to start the batch: ").strip()
    if answer != "RUN_P4D":
...
def open_manifest(args: argparse.Namespace) -> tuple[Path, csv.DictWriter, object]:
    ...
    path = args.out_dir / f"p4d_{scan_label}_batch_manifest_{stamp}.csv"
