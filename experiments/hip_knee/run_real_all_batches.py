...
def build_batches(args: argparse.Namespace) -> list[BatchSpec]:
    ...
    batches = [
        BatchSpec(
            name="p1",
            script=REPO_ROOT / "experiments" / "hip_knee" / "run_real_p1_batch.py",
            args=[],
            description="P1 PD/EID no-disturbance baseline",
        ),
        BatchSpec(
            name="p2",
            script=REPO_ROOT / "experiments" / "hip_knee" / "run_real_p2_batch.py",
            args=[
                "--target",
                args.p2_target,
                "--disturbance-method",
        ...
        BatchSpec(
            name="p3",
            script=REPO_ROOT / "experiments" / "hip_knee" / "run_real_p3_mpc_pd_batch.py",
            args=[],
            description="P3 PD three-point MPC vs four-point velocity-MPC comparison",
        ),
        BatchSpec(
            name="p4",
            script=REPO_ROOT / "experiments" / "hip_knee" / "run_real_p4_batch.py",
            args=p4_args,
