...
from scripts.analysis.analyze_hip_knee_mutual_coupling import (  # noqa: E402
    HIP,
    KNEE,
...
from scripts.reporting.report_paths import analysis_report_dir, markdown_relpath, repo_relpath  # noqa: E402
...
def method_base_config(method: str) -> Path:
    if method == "pd":
        return ROOT / "experiments" / "hip_knee" / "configs" / "h1_real_p2_anti_hip_knee_pd.yaml"
    if method == "eid":
        return ROOT / "experiments" / "hip_knee" / "configs" / "h1_real_p2_anti_hip_knee_eid.yaml"
...
def run_mujoco_case(method: str, freq: float, config: Path, out_dir: Path, args: argparse.Namespace) -> Path:
    ...
    cmd = [
        sys.executable,
        str(ROOT / "scripts" / "simulation" / "run_mujoco.py"),
        "--config",
        str(config),
        "--out-dir",
        str(case_dir),
...
def main() -> int:
    ...
    inv = HipKneeInverseDynamics(ROOT / "models" / "mujoco" / "h1" / "h1.xml")
    details = []
    rows = []
    for method in ["pd", "eid"]:
        for freq in freqs:
