#!/usr/bin/env python3
"""Run the real P3 hip-knee reference-generation experiment batch.
...
DEFAULT_OUT_DIR = REPO_ROOT / "analysis_artifacts" / "real_p3_batch"
...
METHODS = {
    "quintic": MethodSpec(
        method="quintic",
        config=REPO_ROOT / "config" / "h1_real_p3_quintic_hip_knee_pd.yaml",
        condition="P3_PD_quintic_stop_same_phase",
    ),
    "preview2": MethodSpec(
        method="preview2",
        config=REPO_ROOT / "config" / "h1_real_p3_preview2_hip_knee_pd.yaml",
        condition="P3_PD_preview_mpc_2point_same_phase",
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run P3 real H1 quintic-stop/Preview-MPC reference experiments."
    )
    parser.add_argument("--execute", action="store_true", help="Actually run h1_direct. Default is dry-run.")
    parser.add_argument("--yes", action="store_true", help="Skip the RUN_P3 confirmation prompt.")
    ...
    parser.add_argument("--repeats", type=int, default=2, help="Repeats per reference method.")
    ...
    parser.add_argument(
        "--order",
        choices=["alternating", "quintic-then-preview2", "preview2-then-quintic"],
        default="alternating",
        help="Batch order. Alternating runs quintic then preview2 for each repeat.",
    )
    parser.add_argument(
        "--methods",
        choices=["both", "quintic", "preview2"],
        default="both",
        help="Subset of reference methods to run.",
    )
...
def selected_methods(args: argparse.Namespace) -> list[str]:
    if args.methods == "quintic":
        return ["quintic"]
    if args.methods == "preview2":
        return ["preview2"]
    return ["quintic", "preview2"]
...
def build_plan(args: argparse.Namespace) -> list[tuple[int, MethodSpec]]:
    ...
    if args.order == "alternating":
        ordered_methods = [m for m in ["quintic", "preview2"] if m in methods]
        for repeat in indices:
            for method in ordered_methods:
                plan.append((repeat, METHODS[method]))
    else:
        if args.order == "quintic-then-preview2":
            ordered_methods = ["quintic", "preview2"]
        else:
            ordered_methods = ["preview2", "quintic"]
        for method in ordered_methods:
...
def confirm_or_exit(args: argparse.Namespace, plan: list[tuple[int, MethodSpec]]) -> None:
    print(f"P3 batch plan: {len(plan)} runs, duration={args.duration:g}s, pause={args.pause:g}s")
    for i, (repeat, spec) in enumerate(plan, start=1):
        print(f"{i:02d}. {spec.method} r{repeat:02d}: {spec.config.relative_to(REPO_ROOT)}")
    ...
    print("\nThis will command the real robot with no external disturbance.")
    print("Ensure suspension/limits, E-stop, and a watcher are ready.")
    answer = input("Type RUN_P3 to start the batch: ").strip()
    if answer != "RUN_P3":
...
def open_manifest(args: argparse.Namespace) -> tuple[Path, csv.DictWriter, object]:
    ...
    path = args.out_dir / f"p3_batch_manifest_{stamp}.csv"
...
def run_batch(args: argparse.Namespace, plan: list[tuple[int, MethodSpec]]) -> Path | None:
    ...
    try:
        for run_no, (repeat, spec) in enumerate(plan, start=1):
            ...
            print(f"\n[{run_no}/{len(plan)}] {spec.method} r{repeat:02d}")
