...
def write_config(
    ...
    lines: list[str] = [
        "# Generated by scripts/simulation/fit_mujoco_eid_params.py from models/mujoco/h1/h1.xml.",
    ...
    for joint_id in ACTIVE_JOINTS:
        ...
        lines.extend(
            [
                ...
                f"      enabled: {'true' if joint_id in RIGHT_LEG_JOINTS else 'false'}",
...
def main() -> int:
    ...
    parser.add_argument("--xml", type=Path, default=Path("models/mujoco/h1/h1.xml"))
    parser.add_argument("--config-out", type=Path, default=Path("config/simulation/h1_full_body_mujoco_fit.yaml"))
