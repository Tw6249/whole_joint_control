...
from scripts.simulation.fit_mujoco_eid_params import DISPLAY_NAMES, gather_joint_info
...
N_MOTORS = 20
# Must match kJointDebugSize in include/controller_interface.hpp.
N_DEBUG_SLOTS = 40

# Protocol: cmd <flags> <q_cmd[20]> <dq_cmd[20]> <tau[20]> <kp[20]> <kd[20]>
#                <debug[0][0..19]> ... <debug[39][0..19]> <joint_flags[20]>
# = 1 (tag) + 1 + 5*20 + 40*20 + 20 = 922 values
...
def main() -> int:
    ...
    parser.add_argument("--scene", type=Path, default=Path("models/mujoco/h1/scene.xml"))
    parser.add_argument("--config", type=Path, default=Path("config/simulation/h1_full_body_mujoco_fit.yaml"))
